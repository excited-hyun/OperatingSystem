...
void list_swap(struct list_elem *a, struct list_elem *b){
	...
	struct list_elem* aPrev = list_prev(a);
	struct list_elem* aNext = list_next(a);
	struct list_elem* bPrev = list_prev(b);
	struct list_elem* bNext = list_next(b);

	//aPrev->a->b->bNext ----->aPrev->b->a->bNext
	if(bPrev == a){
		aPrev->next = b;
		b->prev = aPrev;
		b->next = a;
	...
	//bPrev->b->a->aNext ----->bPrev->a->b->aNext
	else if(aPrev == b){
		bPrev->next = a;
		a->prev = bPrev;
		a->next = b;
	...
	//---aPrev->a->aNext------bPrev->b->bNext
	//---aPrev->b->aNext------bPrev->a->bNext
	else{
		aPrev->next = b;
		aNext->prev = b;
...
void list_shuffle(struct list* list){
	...
	int random;
	struct list_elem *shuffle1;
	struct list_elem *shuffle2;
	...
	for(int i=0; i<size; i++){
		...
		shuffle1 = list_begin(list);	 //i번째
		for(int j=0; j<i; j++){
			shuffle1 = list_next(shuffle1);
		}
		
		shuffle2 = list_begin(list);	//random번째
		for(int j=0; j<random; j++){
			shuffle2 = list_next(shuffle2);
		}

		list_swap(shuffle1, shuffle2);	//둘이 swap해줌
