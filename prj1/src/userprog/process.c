...
process_wait (tid_t child_tid) 
{
    ...
    int child_cnt = 0, count=0;
    struct list_elem *e = list_begin(&current->child_list);
    bool flag = false;

    child_cnt = list_size(&thread_current()->child_list);
   ...
   child->parent = thread_current();
    ...
    waiting += 1;
    if (waiting > 2) return -1;
    
   while(1) {
	
	   if (busy_waiting(child) == 0)
		   timer_sleep(1);
	   else 
		   break;
   }
