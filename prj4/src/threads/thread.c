...
bool priority_compare(const struct list_elem *one, const struct list_elem *two);
...
bool priority_compare(const struct list_elem *one, const struct list_elem *two){
	...
	thread_one = list_entry(one, struct thread, elem);
	thread_two = list_entry(two, struct thread, elem);
...
thread_init (void) 
{
	...
	list_init(&initial_thread->child_status);
	...
	initial_thread->zombie= false;
	sema_init(&(initial_thread->s_load),0);
	sema_init(&(initial_thread->s_fin),0);
	sema_init(&(initial_thread->s_exit),0);

	for(i=0; i<200; i++)
		initial_thread->file_des[i] =NULL;
	initial_thread->file_cnt=0;
...
init_thread (struct thread *t, const char *name, int priority)
{
	...
	/* Push t to parent's child list */
	t->parent = running_thread();
	list_init(&t->child_status);	
	list_init(&t->child);
	
	t->zombie=false;
	sema_init(&t->s_load, 0);
	sema_init(&t->s_fin, 0);
	sema_init(&t->s_exit, 0);
