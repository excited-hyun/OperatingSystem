...
page_fault (struct intr_frame *f) 
{
  ...
  if (!not_present || !user || fault_addr==NULL)
	  exit(-1);
  ...
  if ( is_user_vaddr(fault_addr) )
  {
	  ...
	  for (range_pnt = PHYS_BASE - PGSIZE; range_pnt >= pg_round_up(fault_addr);range_pnt = range_pnt - PGSIZE){
		  if ( pagedir_get_page( thread_current()->pagedir, range_pnt ) == NULL )
			  break;
		  --page_num;
	  }

      if(pg_round_down(f->esp - 4) != grow_ps && pg_round_down(f->esp - 32) != grow_ps) 
		  exit(-1);
  ...
  /*
  if(!user || is_kernel_vaddr(fault_addr) || !is_user_vaddr(fault_addr)|| not_present)
	  exit(-1);
